...
/**
 * Script corrigido baseado no mapeamento de colunas
 * Resolve o problema de incompatibilidade entre CSV e banco
 */
  ...
  if (eleicaoId) {
    ...
    for (let i = 0; i < dadosEleicao.length; i += batchSize) {
      ...
      for (const dado of batch) {
        ...
        // Preparar dados do perfil - EXATAMENTE 32 valores (31 do CSV + municipio_id + eleicao_id)
        const perfilData = [
      ...
      if (registrosParaInserir.length > 0) {
        ...
        for (let j = 0; j < registrosParaInserir.length; j += subBatchSize) {
          ...
          const values = subBatch.map((_, index) => {
            const offset = index * 32; // 32 valores (31 do CSV + municipio_id + eleicao_id)
