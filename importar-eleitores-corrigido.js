...
/**
 * Script corrigido para importar dados de perfil do eleitor por seção
 * Versão com query SQL corrigida
 */
...
const CONFIG = {
    ...
    QUERY_TIMEOUT: 60000,
    PROGRESS_INTERVAL: 100, // Mostrar progresso a cada 100 linhas
    VALIDATE_DATA: true,
 ...
 */
async function importarDadosEleitoresCorrigido(arquivoCsv, anoEleicao, opcoes = {}) {
    console.log(`🚀 Iniciando importação CORRIGIDA dos dados de eleitores ${anoEleicao}...`);
    ...
    try {
        ...
        // Preparar ambiente
        await prepararAmbienteCorrigido(anoEleicao);
        
        // Processar arquivo CSV
        await processarArquivoCSVCorrigido(arquivoCsv, anoEleicao, batchSize, estatisticas, batch);
        
        // Processar batch restante
        if (batch.length > 0) {
            await processarBatchCorrigido(batch, estatisticas);
 ...
 * Preparar ambiente
 */
async function prepararAmbienteCorrigido(anoEleicao) {
    console.log('🔧 Preparando ambiente...');
    
    // Verificar/criar eleição
    const eleicao = await executarComRetryCorrigido(async () => {
    ...
    if (eleicao.rows.length === 0) {
        ...
        await executarComRetryCorrigido(async () => {
    ...
    await carregarCacheMunicipiosCorrigido();
 ...
 * Carregar cache de municípios
 */
async function carregarCacheMunicipiosCorrigido() {
    console.log('📋 Carregando cache de municípios...');
    
    const municipios = await executarComRetryCorrigido(async () => {
 ...
 * Processar arquivo CSV
 */
async function processarArquivoCSVCorrigido(arquivoCsv, anoEleicao, batchSize, estatisticas, batch) {
    return new Promise((resolve, reject) => {
        ...
        stream.on('data', async (row) => {
            try {
                ...
                // Processar linha
                const dados = processarLinhaCorrigido(row, anoEleicao, estatisticas);
 ...
 */
function processarLinhaCorrigido(row, anoEleicao, estatisticas) {
    try {
 ...
 */
async function processarBatchCorrigido(batch, estatisticas) {
 ...
 */
async function executarComRetryCorrigido(operacao, maxTentativas = CONFIG.MAX_RETRIES) {
...
function gerarRelatorioFinal(estatisticas, tempoTotal) {
    console.log('\n🎉 Importação CORRIGIDA concluída!');
...
async function main() {
    try {
        console.log('🚀 Iniciando importação CORRIGIDA dos dados de eleitores...\n');
        ...
        if (fs.existsSync(arquivo2018)) {
            ...
            const resultado2018 = await importarDadosEleitoresCorrigido(arquivo2018, 2018);
        ...
        if (fs.existsSync(arquivo2022)) {
            ...
            const resultado2022 = await importarDadosEleitoresCorrigido(arquivo2022, 2022);
            console.log(`✅ 2022: ${resultado2022.linhasInseridas} registros inseridos`);
        } else {
            console.log('⚠️  Arquivo de 2022 não encontrado');
        }

        console.log('\n🎉 Importação CORRIGIDA concluída com sucesso!');
...
module.exports = { importarDadosEleitoresCorrigido };
