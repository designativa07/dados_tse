...
/**
 * SOLUÇÃO DEFINITIVA: Script baseado exatamente no método funcional de upload CSV
 * Usa o mesmo padrão que funciona para votos e candidatos
 */

// Função para processar uma linha do CSV de perfil do eleitor
function processarLinhaCSVPerfil(data, lineNumber) {
  try {
    // Limpar e converter dados - EXATAMENTE como no método funcional
    const dados = {
...
// Função principal baseada EXATAMENTE no método funcional
async function processarDadosPerfil(dados, eleicaoId) {
  ...
  if (eleicaoId) {
    ...
    // Processar em lotes para otimizar performance - EXATAMENTE como no método funcional
    console.log(`🔄 Processando ${dadosEleicao.length} registros em lotes...`);
    const batchSize = 1000; // Processar 1.000 registros por vez
    ...
    for (let i = 0; i < dadosEleicao.length; i += batchSize) {
      ...
      // Buscar municípios em lote (otimização) - EXATAMENTE como no método funcional
      ...
      // Preparar dados para inserção em lote - EXATAMENTE como no método funcional
      ...
      for (const dado of batch) {
        ...
        // Preparar dados do perfil - EXATAMENTE como no método funcional
        const perfilData = [
      ...
      // Inserir registros em sub-lotes para evitar limite de parâmetros do PostgreSQL - EXATAMENTE como no método funcional
