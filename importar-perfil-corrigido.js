...
/**
 * Script corrigido baseado no método funcional de upload CSV
 * Resolve o problema de incompatibilidade entre colunas e valores
 */
...
// Função principal baseada no método funcional - CORRIGIDA
async function processarDadosPerfil(dados, eleicaoId) {
  ...
  if (eleicaoId) {
    ...
    for (let i = 0; i < dadosEleicao.length; i += batchSize) {
      ...
      // Inserir registros um por um para evitar problemas de parâmetros
      for (const dado of batch) {
        ...
        try {
          // CORREÇÃO: Query INSERT com número correto de parâmetros
          await db.query(`
