...
const CONFIG = {
    BATCH_SIZE: 200, // Reduzido ainda mais para evitar timeout
    MAX_RETRIES: 5,
    RETRY_DELAY: 2000, // 2 segundos
    CONNECTION_TIMEOUT: 60000, // 60 segundos
    QUERY_TIMEOUT: 300000, // 5 minutos
    PAUSE_BETWEEN_BATCHES: 100 // Pausa entre batches em ms
};
...
async function importarPerfilEleitor(arquivoCsv, anoEleicao) {
    console.log(`🚀 Iniciando importação otimizada do perfil do eleitor ${anoEleicao}...`);
    console.log(`📁 Arquivo: ${arquivoCsv}`);
    console.log(`⚙️  Configuração: Batch size ${CONFIG.BATCH_SIZE}, Timeout ${CONFIG.QUERY_TIMEOUT}ms`);
            ...
            try {
                ...
                if (batch.length >= batchSize) {
                    ...
                    if (linhasProcessadas % 5000 === 0) {
                        ...
                        console.log(`📊 Processadas: ${linhasProcessadas} linhas | Pool: ${stats.idleCount}/${stats.totalCount} conexões`);
                    }
                }
...
async function main() {
    try {
        console.log('🚀 Iniciando importação otimizada dos dados de perfil do eleitor...\n');
